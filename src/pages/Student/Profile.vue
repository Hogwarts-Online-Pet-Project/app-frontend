...
<template>
    <header class="h-[96px] z-10 fixed top-0 left-0 w-screen bg-theme-pearl flex items-center justify-center text-center">
        <p class="text-2xl font-xl">Профиль</p>
    </header>
            ...
            <div class="flex flex-col py-8 w-[70%] max-w-[700px] card_group">
                ...
                <form class="flex flex-col" action="" @submit.prevent="form_submit">
                    <input v-model="old_password"
                        ...
                        placeholder="Текущий пароль" type="password" name="answer" autocomplete="off" required>
                    <input v-model="new_password"
                        class="h-16 mt-4 text-lg text-center text-white outline-none bg-theme-red rounded-xl placeholder:text-white placeholder:opacity-40 placeholder:text-lg"
                        placeholder="Новый пароль" type="password" name="answer" autocomplete="off" required>
                    <input v-model="new_password_confirmation"
                        class="h-16 mt-4 text-lg text-center text-white outline-none bg-theme-red rounded-xl placeholder:text-white placeholder:opacity-40 placeholder:text-lg"
                        placeholder="Подтверждение пароля" type="password" name="answer" autocomplete="off" required>
                    <input type="submit"
                ...
                <a v-if="userStore?.profile?.contract?.Contract_File"
                    :href="`/api/v1/uploads/${userStore?.profile?.contract?.Contract_File}`"
